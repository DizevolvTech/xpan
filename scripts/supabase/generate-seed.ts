...
seedLines.push(
  `insert into public.operational_settings (order_cutoff_time, expedition_lead_days) values (${sqlString(operationalSettings.orderCutoffTime)}, ${operationalSettings.expeditionLeadDays});`,
);
seedLines.push("");
...
seedLines.push(
  `insert into public.categories (legacy_id, code, name, responsible, status) values\n${productionSectors
    .map(
      (sector) =>
        `  (${sqlString(sector.id)}, ${sqlString(sector.code)}, ${sqlString(sector.name)}, ${sqlString(sector.responsible)}, ${sqlString(sector.status)}::public.record_status)`,
    )
    .join(",\n")};`,
);
seedLines.push("");
...
seedLines.push(
  `insert into public.subcategories (legacy_id, code, name, category_id, type, operating_hours, capacity_per_day_kg, status) values\n${productionLines
    .map(
      (line) =>
        `  (${sqlString(line.id)}, ${sqlString(line.code)}, ${sqlString(line.name)}, (select id from public.categories where legacy_id = ${sqlString(line.sectorId)}), ${sqlString(line.type)}::public.line_type, ${sqlString(line.operatingHours)}, ${line.capacityPerDayKg}, ${sqlString(line.status)}::public.record_status)`,
    )
    .join(",\n")};`,
);
seedLines.push("");
...
seedLines.push(
  `insert into public.stores (legacy_id, code, name, responsible, email, phone, status, receive_window, ordering_days, receiving_days) values\n${storesMasterData
    .map(
      (store) =>
        `  (${sqlString(store.id)}, ${sqlString(store.code)}, ${sqlString(store.name)}, ${sqlString(store.responsible)}, ${sqlString(store.email)}, ${sqlString(store.phone)}, ${sqlString(store.status)}::public.record_status, ${sqlString(store.receiveWindow)}, ${sqlArray(store.orderingDays, "public.weekday_code")}, ${sqlArray(store.receivingDays, "public.weekday_code")})`,
    )
    .join(",\n")};`,
);
seedLines.push("");
...
seedLines.push(
  `insert into public.ingredients (legacy_id, code, name, type, unit, metadata, observation, status) values\n${productionIngredients
    .map(
      (ingredient) =>
        `  (${sqlString(ingredient.id)}, ${sqlString(ingredient.code)}, ${sqlString(ingredient.name)}, ${sqlString(ingredient.type)}::public.ingredient_type, ${sqlString(ingredient.unit)}::public.unit_code, ${sqlString(ingredient.metadata)}, ${sqlString(ingredient.observation)}, ${sqlString(ingredient.status)}::public.record_status)`,
    )
    .join(",\n")};`,
);
seedLines.push("");
    ...
    `insert into public.ingredient_components (ingredient_id, ingredient_reference_id, product_reference_id, name, quantity, unit, observation, sort_order) values\n${ingredientComponents
      .map(
        ({ ingredient, component, index }) =>
          `  ((select id from public.ingredients where legacy_id = ${sqlString(ingredient.id)}), ${
...
if (recipeItems.length > 0) {
  seedLines.push(
    `insert into public.product_recipe_items (product_id, source_type, ingredient_source_id, product_source_id, label, quantity, unit, sort_order) values\n${recipeItems
      .map(
        ({ product, item, index }) =>
          `  ((select id from public.products where legacy_id = ${sqlString(product.id)}), ${sqlString(item.sourceType)}::public.recipe_source_type, ${
...
seedLines.push(
  `insert into public.schedule_lines (legacy_id, code, name, subcategory_id, revision_of_id, status, created_at, created_by_profile_id, audited_at, audited_by_profile_id, audit_notes, deactivated_at, deactivated_by_profile_id) values\n${weeklySchedules
    .map(
      (schedule) =>
        `  (${sqlString(schedule.id)}, ${sqlString(schedule.code)}, ${sqlString(schedule.name)}, (select id from public.subcategories where legacy_id = ${sqlString(schedule.lineId)}), ${
...
if (scheduleSnapshots.length > 0) {
  seedLines.push(
    `insert into public.schedule_line_item_snapshots (schedule_line_id, product_id, minimum_production, production_days) values\n${scheduleSnapshots
      .map(
        ({ schedule, item }) =>
          `  ((select id from public.schedule_lines where legacy_id = ${sqlString(schedule.id)}), (select id from public.products where legacy_id = ${sqlString(item.productId)}), ${item.minimumProduction}, ${sqlArray(item.productionDays, "public.weekday_code")})`,
      )
      .join(",\n")};`,
  );
  seedLines.push("");
...
seedLines.push(
  `insert into public.user_permissions (profile_id, module_key, access_level) values\n${permissionRows
    .map(
      (row) =>
        `  ((select id from public.profiles where legacy_id = ${sqlString(row.userId)}), ${sqlString(row.moduleKey)}, ${sqlString(row.accessLevel)}::public.permission_level)`,
    )
    .join(",\n")};`,
);
seedLines.push("");
...
seedLines.push(
  `insert into public.profile_store_access (profile_id, store_id) values\n${storesMasterData
    .map(
      (store) =>
        `  ((select id from public.profiles where legacy_id = 'user-loja'), (select id from public.stores where legacy_id = ${sqlString(store.id)}))`,
    )
    .join(",\n")};`,
);
seedLines.push("");
...
seedLines.push(
  `insert into public.store_order_items (legacy_id, order_id, product_id, product_code_snapshot, product_name_snapshot, requested_quantity, requested_unit, sales_to_kg_factor_snapshot, internal_kg_snapshot, expedition_unit_snapshot, expedition_to_kg_factor_snapshot, operational_unit_snapshot) values\n${storeOrderItemRows
    .map(
      (row) =>
        `  (${sqlString(row.item.id)}, (select id from public.store_orders where legacy_id = ${sqlString(row.order.id)}), (select id from public.products where legacy_id = ${sqlString(row.product.id)}), ${sqlString(row.product.code)}, ${sqlString(row.product.name)}, ${row.item.quantity}, ${sqlString(row.item.unit)}::public.unit_code, ${row.product.salesToKgFactor}, ${row.internalKg}, ${sqlString(row.product.expeditionUnit)}::public.unit_code, ${row.product.expeditionToKgFactor}, ${sqlString(row.product.productionUnit)}::public.unit_code)`,
    )
    .join(",\n")};`,
);
seedLines.push("");
...
seedLines.push(
  `insert into public.workflow_order_releases (order_id, released_at, released_by_profile_id) values\n${releasedOrderEntries
    .map(
      (entry) =>
        `  ((select id from public.store_orders where legacy_id = ${sqlString(entry.orderId)}), ${sqlString(entry.releasedAt)}, (select id from public.profiles where legacy_id = 'user-fabrica'))`,
    )
    .join(",\n")};`,
);
seedLines.push("");
...
if (workflowItems.length > 0) {
  seedLines.push(
    `insert into public.workflow_production_items (production_item_key, status, progress, updated_at, updated_by_profile_id) values\n${workflowItems
      .map(
        (item) =>
          `  (${sqlString(item.key)}, ${sqlString(item.status)}::public.production_item_status, ${item.progress}, ${sqlString("2026-03-09T12:00:00Z")}, (select id from public.profiles where legacy_id = 'user-chao'))`,
      )
      .join(",\n")};`,
  );
  seedLines.push("");
...
seedLines.push(
  `insert into public.delivery_executions (order_id, status, updated_at, updated_by_profile_id) values\n${deliveryEntries
    .map(
      (entry) =>
        `  ((select id from public.store_orders where legacy_id = ${sqlString(entry.orderId)}), ${sqlString(entry.status)}::public.delivery_execution_status, ${sqlString(entry.updatedAt)}, (select id from public.profiles where legacy_id = 'user-fabrica'))`,
    )
    .join(",\n")};`,
);
seedLines.push("");
...
seedLines.push(
  `insert into public.store_occurrences (legacy_id, code, order_id, order_item_id, product_id, product_name_snapshot, problem_type, quantity_type, quantity, quantity_unit_snapshot, description, status, opened_by_profile_id, resolved_by_profile_id, resolved_at, created_at, updated_at) values\n${storeOccurrences
    .map(
      (occurrence) =>
        `  (${sqlString(occurrence.id)}, ${sqlString(occurrence.code)}, (select id from public.store_orders where legacy_id = ${sqlString(occurrence.orderId)}), (select id from public.store_order_items where legacy_id = ${sqlString(occurrence.orderItemId)}), (select id from public.products where legacy_id = ${sqlString(occurrence.productId)}), ${sqlString(occurrence.productName)}, ${sqlString(occurrence.problemType)}, ${sqlString(occurrence.quantityType)}::public.occurrence_quantity_type, ${occurrence.quantity}, ${sqlString(occurrence.quantityUnit)}, ${sqlString(occurrence.description)}, ${sqlString(occurrence.status)}::public.occurrence_status, (select id from public.profiles where legacy_id = 'user-loja'), ${
