...
import { ArrowLeft, ClipboardList, Factory, Layers, Printer, ShoppingCart, Truck } from "lucide-react";
...
export default function OrdensProducaoPage() {
  ...
  const opActions = [
    {
      icon: "view" as const,
      label: "Visualizar OP",
      onClick: (item: OpQueueRow) => router.push(`/gestor-fabrica/ordens-producao/${item.id}?ref=${referenceDate}`),
    },
    {
      icon: "print" as const,
      label: "Imprimir OP",
  ...
  return (
    <PageLayout
      ...
      description="A necessidade por produto é processada em segundo plano. Aqui você acompanha somente as OPs já consolidadas."
      badge="Fábrica"
      breadcrumbs={[
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <KPICard title="Setores Ativos" value={kpis.setoresAtivos} icon={ShoppingCart} tone="info" compactValue />
      ...
      <FactoryFlow
        currentKey="producao"
        steps={flowSteps}
        subtitle="A necessidade por produto roda no background. Nesta tela você foca apenas na execução das OPs."
      />
