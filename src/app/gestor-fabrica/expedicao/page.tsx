...
import { ArrowLeft, Factory, ListChecks, Package, Printer, ShoppingCart, Truck } from "lucide-react";
...
export default function ExpedicaoPage() {
      ...
      render: (item: ExpeditionOrderRow) => {
        ...
        return (
          <Button asChild type="button" variant="outline" size="sm">
            <Link href={`/gestor-fabrica/expedicao/${item.id}?ref=${referenceDate}`}>Abrir separação</Link>
          </Button>
        );
      },
    },
  ...
  return (
    <PageLayout
      title="Expedição"
      description="Selecione o pedido na tabela e abra a tela dedicada de separação e reconversão."
      badge="Fábrica"
      breadcrumbs={[
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <KPICard title="Entregas no dia" value={kpis.entregasHoje} icon={ShoppingCart} tone="warning" compactValue />
      </div>
      ...
      <FactoryFlow
        ...
        steps={flowSteps}
        subtitle="Fluxo sugerido: selecionar o pedido na tabela e abrir a separação em tela dedicada."
      />
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-3">
          ...
          <div className="rounded-lg border border-border/70 bg-panel/45 p-3 text-sm">
            ...
            <p className="mt-1 font-medium text-foreground">
              Atualize o status do pedido e gere a impressão operacional para execução no chão de fábrica.
            </p>
          </div>
        </CardContent>
      </Card>
